...
function addMsg(msg, prepend) {
    ...
    if(msg.elems[0].type === 'TIMCustomElem') {
        try {
            var data = JSON.parse(msg.elems[0].content.data);
            if(data.action === 60400) {
                if(window.shareUrl !== '') {
                    document.getElementById('link_layer').style.display = 'block';
                    if(window.sendLog) {
                        window.sendLog('aizbk393');
                    }
                } else {
                    window.location.href= '/cathedraDetail' + location.search;
                }
            } else if(data.action === 10900) {
                if(window.showToast) showToast('老师关闭聊天了，安静听课吧～')
            } else if(data.action === 10950) {
                if(window.showToast) showToast('老师开放聊天啦，和大家说两句吧');
            }
        } catch(e) {
            console.log(e);
        }
    ...
    if(!isSelfSend && window.seeYourself) return;
    ...
    onemsg.className = "onemsg c-item";
    var msgAvator = document.createElement("div");
    var msgBody = document.createElement("div");
    var msgWrap = document.createElement("span");
    var msgName = document.createElement("span");
    var msgPre = document.createElement("span");
    msgAvator.className = "c-avator";
    msgBody.className = "c-info";
    msgWrap.className = "c-info-box";
    msgName.className = "c-info-name";
    msgPre.className = "c-info-text"


    //如果是发给自己的消息
    if (isSelfSend)
        onemsg.className += ' c-right';
    //昵称  消息时间
    msgAvator.innerHTML = "<img src='" + fromAccountImage + "'>";
    if (isSelfSend) {
      msgName.innerHTML = webim.Tool.formatText2Html(' :' + fromAccountNick);
    } else {
      msgName.innerHTML = webim.Tool.formatText2Html(fromAccountNick + ':');
    }
    ...
    msgBody.appendChild(msgWrap);
    if (isSelfSend) {
      msgWrap.appendChild(msgPre);
      msgWrap.appendChild(msgName);
    } else {
      msgWrap.appendChild(msgName);
      msgWrap.appendChild(msgPre);
    }

    // onemsg.appendChild(msgAvator);
    // onemsg.appendChild(msgBody);//暂时注释
    // 暂时添加
    if(convertMsgtoHtml(msg) !=""){
      onemsg.appendChild(msgBody);
    }
    //消息列表
    var msgflow = document.getElementsByClassName("msgflow")[0];
    var msgtip = msgflow.getElementsByClassName("tip")[0];
    msgtip && msgflow.removeChild(msgtip);
    if (prepend) {
        ...
        if(msgflow.children.length > 100) {
            msgflow.removeChild(msgflow.children[0]);
        }
    ...
    for (var i = 0; i < count; i++) {
        ...
        switch (type) {
            case webim.MSG_ELEMENT_TYPE.TEXT:
                ...
                //转义，防XSS
                html += replaceMapEmoji(webim.Tool.formatText2Html(eleHtml));
                break;
...
function convertCustomMsgToHtml(content) {
    ...
    // return "data=" + data + ", desc=" + desc + ", ext=" + ext;//暂时注释掉
    return "";// 暂时添加
}
//解析群提示消息元素
